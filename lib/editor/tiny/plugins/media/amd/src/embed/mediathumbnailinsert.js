...
/**
 * Tiny media plugin media thumbnail upload class for embed.
 *
 * @module      tiny_media/embed/mediathumbnailinsert
...
import {MediaThumbnailPreview} from './mediathumbnailpreview';
...
export class MediaThumbnailInsert extends PropertySetter {
    ...
    init = async(mediaData) => {
        this.mediaData = mediaData; // Current selected media data passed from mediaDetails.
    ...
    loadPreviewThumbnail = (url) => {
        ...
        Promise.all([body(templateContext, this.thumbnailModalRoot), footer(templateContext, this.thumbnailModalRoot)])
            .then(() => {
                const mediaThumbnail = new MediaThumbnailPreview(this);
