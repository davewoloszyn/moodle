...
class registration {
    ...
    public static function get_ai_usage_data(): array {
        ...
        $sql = "SELECT aar.*, COALESCE(aagt.model, aast.model, aagi.model) AS model
                  FROM {ai_action_register} aar
             LEFT JOIN {ai_action_generate_text} aagt
                    ON aar.actionid = aagt.id AND aar.actionname = 'generate_text'
             LEFT JOIN {ai_action_summarise_text} aast
                    ON aar.actionid = aast.id AND aar.actionname = 'summarise_text'
             LEFT JOIN {ai_action_generate_image} aagi
                    ON aar.actionid = aagi.id AND aar.actionname = 'generate_image'
