...
abstract class abstract_processor extends process_base {
    ...
    protected function handle_api_error(ResponseInterface $response): array {
        ...
        if ($status >= 500 && $status < 600) {
            $responsearr['errormessage'] = $reason ?: 'Internal server error';
            ...
            $responsearr['errormessage'] = $bodyobj->error->message ?? $reason ?: 'An error occurred';
            $responsearr['error'] = $bodyobj->error->status ?? 'ERROR';
            return $responsearr;
        }
    }
}
