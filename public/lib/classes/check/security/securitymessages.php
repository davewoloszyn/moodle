    ...
    public function get_result(): result {
        ...
        foreach (self::REQUIRED_FORCED as $providerkey) {
            ...
            foreach ($enabledprocessors as $processor) {
                ...
                $matrix[$providerkey][$processor->name] = ['enabled' => $enabled, 'locked' => $locked];

                if (!$enabled || !$locked) {
                    $notok[] = $component . '/' . $name . ' (' . $processor->name . ')';
                }
            }
        }
        ...
        $table->head = array_merge([get_string('notifications', 'message')], $processorlabels);
        $table->data = [];

        foreach ($matrix as $providerkey => $processors) {
            $row = [\html_writer::tag('code', $providerkey)];
