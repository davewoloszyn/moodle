...
final class manager_test extends \advanced_testcase {
    ...
    public function test_set_scheduled_task_nextruntime(): void {
        ...
        $first = \core\task\manager::get_next_scheduled_task(time());
        $this->assertNotNull($first);
        manager::scheduled_task_complete($first);

        $next = \core\task\manager::get_next_scheduled_task(time() + 10);
        $this->assertNull($next);

        manager::set_scheduled_task_nextruntime($first, time() + 20);

        $next = \core\task\manager::get_next_scheduled_task(time() + 30);
